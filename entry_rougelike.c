...
int tile_pos_to_world_pos(float tile_pos) {
...
Vector2 screen_to_world() {
	...
	float ndcY = (mouse_y / (window_h * 0.5f)) - 1.0f;
...
int entry(int argc, char **argv) {
	...
	sprites[SPRITE_player] = (Sprite){ .image=load_image_from_disk(STR("player.png"), get_heap_allocator()), .size=v2(13.0f, 18.0f) };
	sprites[SPRITE_slime] = (Sprite){ .image=load_image_from_disk(STR("slime.png"), get_heap_allocator()), .size=v2(9.0, 7.0) };
	sprites[SPRITE_tree] = (Sprite){ .image=load_image_from_disk(STR("tree.png"), get_heap_allocator()), .size=v2(18.0f, 30.0f) };
			...
			for (int x = player_tile_x - tile_radius_x; x < player_tile_x + tile_radius_x; x++) {
				for (int y = player_tile_y - tile_radius_y; y < player_tile_y + tile_radius_y; y++) {
					if ((x + (y % 2 == 0) ) % 2 == 0) {
						...
						draw_rect(v2(x_pos + (float)tile_width * -0.5, y_pos + (float)tile_width * -0.5), v2(tile_width, tile_width), v4(0.1, 0.1, 0.1, 0.1));
					}
				}
			}
			...
			if (en->is_valid) {
				...
				switch (en->arch) {		
					...
					default:
					{
						...
						draw_text(font, sprint(get_temporary_allocator(), tprint("%f %f" , en->pos.x, en->pos.y)), font_height, en->pos, v2(0.1,0.1), COLOR_WHITE);
